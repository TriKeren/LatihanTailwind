...
const JoinZebb = () => {
    ...
    return ( 
        <div className="max-w-screen-xl mx-auto flex justify-center mt-10 lg:mt-14">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 py-20 rounded-lg">
                <div 
                    className={`flex justify-center order-1 lg:order-2 lg:text-right transform transition-opacity duration-500 ${imageVisible ? 'opacity-100' : 'opacity-0'}`}
                    ref={imageRef}
                >
                    <img src="/public/assets/ZebbDigitalAgency/meditation.png" className="w-11/12 mx-10 mt-16" alt="" />
                </div>
                <div 
                    className={`mx-14 text-black transform transition-transform duration-500 ${imageVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}
                >
